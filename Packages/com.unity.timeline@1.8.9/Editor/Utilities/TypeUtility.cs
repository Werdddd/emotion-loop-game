...
namespace UnityEditor.Timeline
{
    ...
    foreach (var trackType in s_AllTrackTypes)
    {
        ...
        if (typeof(MonoBehaviour).IsAssignableFrom(trackType) && !trackType.IsAbstract)
        {
            var temp = (MonoBehaviour)new GameObject().AddComponent(trackType);
            script = MonoScript.FromMonoBehaviour(temp);
            UnityEngine.Object.DestroyImmediate(temp.gameObject);
        }
        else if (typeof(ScriptableObject).IsAssignableFrom(trackType) && !trackType.IsAbstract)
        {
            var temp = ScriptableObject.CreateInstance(trackType);
            script = MonoScript.FromScriptableObject(temp);
